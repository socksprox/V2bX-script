...
show_status() {
    ...
    case $? in
        0)
            echo -e "V2bX status: ${green}Operational${plain}"
            show_enable_status
            ;;
        1)
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Autoboot: ${green}Enabled${plain}"
    else
        echo -e "Autoboot: ${red}Disabled${plain}"
    fi
}
...
add_node_config() {
    echo -e "${green}Please select the Node core configuration type：${plain}"
    ...
    if [[ "$isreality" != "y" && "$isreality" != "Y" && ( "$istls" == "y" || "$istls" == "Y" ) ]]; then
        echo -e "${yellow}Select certificate application mode：${plain}"
        echo -e "${green}1. http The schema is automatically applied and the node domain name is correctly resolved${plain}"
        ...
        echo -e "${green}3. self Mode, self-signed certificates or providing documentation of existing certificates${plain}"
        ...
        read -rp "Enter the domain name for the Node certificate(example.com)]：" certdomain
        if [ "$certmode" != "http" ]; then
            echo -e "${red}Please restart V2bX after modifying the configuration file manually！${plain}"
        fi
    fi
...
generate_config_file() {
    echo -e "${yellow}V2bX Configuration File Generation${plain}"
    echo -e "${red}Please read the following notes：${plain}"
    echo -e "${red}1. The feature is currently in beta${plain}"
    echo -e "${red}2. Generate config is saved to /etc/V2bX/config.json${plain}"
    echo -e "${red}3. Original config is backed up to /etc/V2bX/config.json.bak${plain}"
    ...
    while true; do
        if [ "$first_node" = true ]; then
            read -rp "Enter your Airport Website(https://example.com)：" ApiHost
            read -rp "Your Airport Api Key：" ApiKey
            read -rp "Set a fixed Aiport URL/API Key？(y/n)" fixed_api
            if [ "$fixed_api" = "y" ] || [ "$fixed_api" = "Y" ]; then
                fixed_api_info=true
                echo -e "${red}Set fixed address successfully${plain}"
            fi
            first_node=false
            add_node_config
        else
            ...
            if [[ "$continue_adding_node" =~ ^[Nn][Oo]? ]]; then
                ...
                read -rp "Please enter Airport Api Key：" ApiKey
            fi
            add_node_config
        fi
    done
